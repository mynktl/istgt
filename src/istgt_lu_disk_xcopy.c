...
istgt_find_xcopy_target(ISTGT_Ptr istgt, uint64_t identifier)
{
	...
	for (i = 0; i < MAX_LOGICAL_UNIT; i++) {
			...
			if (lu == NULL)
				continue;
			/*
			 * Cloudbyte supports single lun for logical group
			 * so zero
			 */
...
istgt_get_xcopy_target(ISTGT_LU_DISK *spec, uint8_t *target_descriptor,
    ISTGT_LU_CMD_Ptr lu_cmd)
{
	...
	if (code_set == 1) {
		ISTGT_TRACELOG(ISTGT_TRACE_DEBUG, "Association and Identifier "
		    "field contains Binary values \n");
	}
	...
	ISTGT_TRACELOG(ISTGT_TRACE_DEBUG, "Association %d, Identifier_type %d, "
	    "Identifier_length %d\n",
	...
	if (identifier_type == NAA_IDENTIFIER) {
		...
		if (naa == NAA_IDENTIFIER) {
			...
			ISTGT_TRACELOG(ISTGT_TRACE_DEBUG, "Locally Assigned "
			    "Descriptor\n");
	...
	if (ripi == 0) {
		/* use any port */
		ISTGT_TRACELOG(ISTGT_TRACE_DEBUG, "Relative Initiator "
		    "Port %d\n", ripi);
	}
    ...
    ISTGT_LU_CMD_Ptr lu_cmd, uint8_t *data_buf, int dlen)
{
	...
	if (spec == NULL) {
		/*
		 * This spec can be source target / destination target,
		 * should not be NULL
		 */
	...
	if (priority == 0) {
		...
		/*
		 * CloudByte: Currently does not support priority on the
		 * commands.
		 */
	}
	...
	if (list_id_usage == LIST_ID_USAGE) {
		 ...
		 * the copy manager need not hold any data for tha
		 * application client, and the Extended copy commands list
	...
	/*
	 * Number of target descriptors must not be more than
	...
	/*
	 * Fetch the segment descriptors to get the index for
	 * target descriptor
	 */
	len = PARAMETER_HEADER_DATA + tdll;

	/*
	 * need to come back to this one later.. if we need to have xcopy
	 * support
	 */
#if 0
	...
	while (len +4 <=  dlen) {
		...
		rc = istgt_lu_disk_lbxcopy(src_tgt, dst_tgt, conn, lu_cmd, dc,
		    cat, num_of_blks_byts, sd_opcode);
		if (rc < 0) {
...
istgt_lu_disk_xcopy(ISTGT_LU_DISK *spec, CONN_Ptr conn, ISTGT_LU_CMD_Ptr lu_cmd)
{
	...
	if (opcode != 0x83) {
		ISTGT_ERRLOG("c#%d EXTENDED COPY command opcode seems "
		    "to be wrong \n", conn->id);
		return (-1);
	}
    ...
    uint64_t num_blks_byts, uint8_t sd_opcode)
{
	...
	if (rc < 0 || (uint64_t) rc != nbytes) {
		 ...
		 * for the next segment processing if the cat
		 * bit is being Non-zero
		 */
	...
	if (rc < 0 || (uint64_t) rc != nbytes) {
		 ...
		 * Preserve the residual data for the next segment processing
		 * if the cat bit being NON-Zero
		 */
